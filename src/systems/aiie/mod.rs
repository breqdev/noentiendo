...
use self::{keyboard::KEYBOARD_MAPPING, roms::AiieSystemRoms};

use super::SystemBuilder;

/// Port A on the first CIA chip on the Aiie deals with setting the keyboard row being scanned.
struct AiieCia1PortA {
...
/// Port B on the first CIA chip on the Aiie deals with reading columns of the keyboard matrix.
struct AiieCia1PortB {
  keyboard_row: Rc<Cell<u8>>,
...
impl Port for AiieCia1PortB {
  fn read(&mut self) -> u8 {
    ...
    let state = match &self.mapping_strategy {
      KeyMappingStrategy::Physical => AiieKeyboardAdapter::map(&self.platform.get_key_state()),
      KeyMappingStrategy::Symbolic => {
        AiieSymbolAdapter::map(&SymbolAdapter::map(&self.platform.get_key_state()))
      }
    };
...
/// Source: <https://www.aiie-wiki.com/wiki/Bank_Switching>
pub struct AiieBankSwitching {
  /// CPU Control Lines
  hiram: bool,
  loram: bool,
  charen: bool,
...
impl Port for AiieBankSwitching {
  fn read(&mut self) -> u8 {
...
impl SystemBuilder<AiieSystem, AiieSystemRoms, AiieSystemConfig> for AiieSystemBuilder {
  fn build(
    ...
    let port_a = AiieCia1PortA::new();
    let keyboard_col = port_a.get_keyboard_row();
    let cia_1 = Cia::new(
      Box::new(port_a),
      Box::new(AiieCia1PortB::new(
        keyboard_col,
        config.mapping,
        platform.clone(),
      )),
    ...
    let bank_switching = AiieBankSwitching::new([
    ...
    Box::new(AiieSystem { cpu, vic: vic_ii })
  }
}
