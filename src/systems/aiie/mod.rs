...
use crate::{
  ...
  memory::{
    BlockMemory, BranchMemory, NullMemory,
    SystemInfo, Memory, ActiveInterrupt,
  },
  platform::{PlatformProvider, WindowConfig},
  systems::System, roms::RomFile,
};
...
impl Memory for AiieSoftSwitches {
    fn read(&mut self, address: u16) -> u8 {
        match address % 0x100 {
          0x00 => todo!("keyboard data"),
          0x01..=0x0F | 0x50..=0x5F => { self.softswitch(address); 0 },
          0x10..=0x1F => self.read_flag(address),
          0x30 => {println!("SPEAKER : toggle speaker diaphragm"); 0},
